function NormalTexturedGeometry(){
    ...
    this.shader = shaderProgramNormalTexturedObject;
    ...
    function handleLoadedNormalMap(model) {
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
        gl.bindTexture(gl.TEXTURE_2D, model.normal_map);

        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, model.normal_map.image);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_NEAREST);
        gl.generateMipmap(gl.TEXTURE_2D);
    ...
    this.initNormalMap = function(texture_file){
        ...
        this.normal_map.image.onload = function () {
            handleLoadedNormalMap(model)
        };
        this.normal_map.image.src = texture_file;
    };
    ...
    this.draw = function(){
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.texture);
        gl.uniform1i(this.shader.textureUniform, 0);

        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, this.normal_map);
        gl.uniform1i(this.shader.normalMapUniform, 1);
